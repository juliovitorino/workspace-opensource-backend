...
@Service
public class FindPersonalTrainerByGuardianIdServiceImpl implements FindPersonalTrainerByGuardianIdService {

    private final UserRepository userRepository;

    public FindPersonalTrainerByGuardianIdServiceImpl(UserRepository userRepository) {
        this.userRepository = userRepository;
    }

    @Override
    public ControllerGenericResponse<PersonalTrainerResponse> execute(UUID processId, UUID uuid) {
        User user = userRepository.findByGuardianIntegrationUUID(uuid)
                .orElseThrow(
