...
public interface StudentPaymentRepository extends JpaRepository<StudentPayment, Long> {
    ...
    @Query("select coalesce( sum(c.amount - coalesce( "+
            ...
            "and c.status = 'A' " +
            "and c.contract.trainingPack.personalUser.id = :personalId")
    BigDecimal sumFutureRevenues(@Param("personalId") Long personalId);
    ...
    @Query(nativeQuery = true,
            value = "WITH bills AS (" +
                    ...
                    "    AND sp.expected_date < CURRENT_DATE " +
    ...
    @Query(nativeQuery = true,
            value = "WITH revenues AS (" +
                    ...
                    "SELECT * " +
                    "FROM student_payments sp " +
                    "WHERE sp.id IN (SELECT id FROM revenues)")
    List<StudentPayment> findAllFutureRevenue(@Param("personalId") Long personalId);
